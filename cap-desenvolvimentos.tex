...
Existem diferentes maneiras de se resolver um problema
Programar com diferentes métodos/Programar apenas em um método

Otimizar baseia-se em escolher melhor método para resolver um problema
Escolher estes métodos é um problema
Costumam ser deixados como escolha na forma de parâmetros

"In our experience in the area of designing high-performance heuristic solvers for various hard combinatorial
problems, building software in this manner leads to suboptimal results in terms of performance and
adaptability to different use contexts." (\cite{hoos2012programming}).

Programar por Otimização é uma maneira de lidar com isso -> Autotuning
Mencionar a plataforma de PbO aqui

Autotuning vem sendo usado em áreas como computação de alta performance e computação gráfica
Encontra o mesmo problema de otimização manual: tuners normalmente são feitos application-specific
Três desafios: Encontrar a representação de parâmetros correta para o programa
	O tamanho do espaço de configurações válido %(tosse tosse 10^30 tosse tosse)
	%Miojo notes: Não comite e push algo que não compila, jovem. A linha de cima quebra. >_> 
	A topografia do espaço de configurações pode ser muito complexa para métodos de busca (simples||específicos)

PetaBricks como uma forma de PbO -> otimiza dados um problema e metódos diferentes de resolvê-lo

OpenTuner como uma solução que otimiza via parâmetros
	qualquer problema que possa ter sua entrada parametrizada e uma saída quantificável
